...
create type if not exists public.gender as enum ('Masculino','Feminino','Não-binário','Prefiro não responder');
create type if not exists public.age_range as enum ('Até 18', '19-25', '26-35', '36-45', '46-60', '60+');
create type if not exists public.answer_type as enum ('text','audio');
create type if not exists public.question_key as enum ('future_vision','magic_wand','what_is_missing');
...
create trigger if not exists submissions_set_updated_at
...
create policy "Stations readable for everyone"
...
create policy "Anon can upload survey files"
  on storage.objects for insert to anon
  with check (bucket_id = 'survey');

create policy "Authenticated can read survey files"
  on storage.objects for select to authenticated
  using (bucket_id = 'survey');

create policy "Authenticated can update survey files"
  on storage.objects for update to authenticated
  using (bucket_id = 'survey')
  with check (bucket_id = 'survey');

create policy "Authenticated can delete survey files"
